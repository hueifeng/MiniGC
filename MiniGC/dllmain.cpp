...
)
{
	...
	{
		std::cout << "[Loader] Detaching loader" << std::endl;
		return true;
	}
	...
	auto module = LoadLibraryA("GarbageCollector.dll");

	if (module == 0)
	{
		std::cout << "Could not find GarbageCollector.dll" << std::endl;
		return FALSE;
	}

	std::cout << "[Loader] Successfully loaded GarbageCollector.dll" << std::endl;
